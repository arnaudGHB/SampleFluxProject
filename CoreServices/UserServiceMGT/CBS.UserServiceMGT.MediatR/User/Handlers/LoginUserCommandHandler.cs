        ...
        public async Task<ServiceResponse<LoginResponse>> Handle(LoginUserCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (user == null || !BCrypt.Net.BCrypt.Verify(request.Password, user.PasswordHash))
                {
                    ...
                    return ServiceResponse<LoginResponse>.Return401("Invalid credentials provided to UserServiceMGT.");
                }
                ...
                return ServiceResponse<LoginResponse>.ReturnResultWith200(response, "Credentials are valid. Token obtained from UserServiceMGT.");
            }
            catch (Exception ex)
            {
                // --- CORRECTION : Utilisation du template de message pour le logging ---
